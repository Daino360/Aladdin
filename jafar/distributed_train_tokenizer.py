...
from functools import partial # to wrap multiple gpu calls
...
# Initialize JAX for multi-GPU and return the number of devices
num_devices = jax.local_device_count()
print(f"Using {num_devices} devices")

# Ensures that the batch size is divisible by the number of GPUs
args.batch_size = args.batch_size // num_devices * num_devices 

# Modify your tokenizer_loss_fn to handle per-device batches
# The loss computation remains the same, but JAX automatically applies it to each GPU's batch.
def tokenizer_loss_fn(params, state, inputs):
    # inputs are now per-device batches (smaller chunk of the full batch).
    ...
    # Metrics calculation remains similar
...
# Create a parallel version of the training step
@partial(jax.pmap, axis_name='batch', donate_argnums=(0,)) # jax.pmap allows parallel execution across devices, Labels the axis along which we parallelize (the batch dimension), Allows JAX to reuse memory for the first argument 
...
# Modify your main training loop
if __name__ == "__main__":
    ...
    while step < args.num_steps:
        for videos in dataloader:
            ...
            batch_size_per_device = videos.shape[0] // num_devices
            videos_sharded = videos.reshape(num_devices, batch_size_per_device, *videos.shape[1:])
            
            rng, _rng = jax.random.split(rng)
            # Split RNG across devices
            _rng = jax.random.split(_rng, num_devices)
            
            inputs = dict(videos=videos_sharded, rng=_rng)
            
            # The pmapped train_step will handle the parallel execution
            train_state, loss, recon, metrics = train_step(train_state, inputs)
            
            # Since we're using pmap, the outputs are sharded - take the mean
            loss = loss.mean()

#!/usr/bin/env python3
"""
PPO training directly inside Procgen CoinRun with the custom flow-odometry goal reward.

- Uses the same flow residual / odometry reward you debugged in coinrun_simulator.py.
- Mirrors the CLI ergonomics of agent_train.py but bypasses the world model.
- Keeps SB3 VecEnv + TensorBoard logging so you can reuse your monitoring workflow.
"""
...
def compute_flow_reward(
    ...
    *,
    ...
    # allow goal_x to move forward if enabled
    gx = state.get("goal_x", gx)
    gy = state.get("goal_y", gy)
    prev_dist = math.sqrt(
    ...
    reward = cfg.reward_scale * (prev_dist - dist)
    if dist < cfg.goal_radius:
        reward += cfg.success_bonus
        if cfg.advance_goal_on_success:
            state["goal_x"] = gx + cfg.goal_radius
    # expose current goal for debugging if needed
    return float(reward), float(dist)


class FlowGoalRewardWrapper(gym.Wrapper):
    """Replace the native Procgen reward with the flow/goal reward."""

    def __init__(self, env: gym.Env, cfg: FlowRewardConfig):
        super().__init__(env)
        self.cfg = cfg
        self._state = None
        self._prev_rgb = None
    ...
    def step(self, action):
        ...
        reward_custom, dist = compute_flow_reward(
            self._prev_rgb,
            curr_rgb,
            cfg=self.cfg,
            state=self._state,
        ...
        # Always expose native_return so outer wrappers (e.g., TimeLimit) don't drop it
...
class TqdmProgressCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if now - self._last_t < self.refresh:
            return True
        steps = max(0, int(self.model.num_timesteps) - self.start_offset)
...
def _make_env_thunk(
    ...
    def _init():
        ...
        env = FlowGoalRewardWrapper(env, reward_cfg)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1_000_000,
        help="Number of SIM timesteps to run (even when warm-starting).",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional SB3 .zip to warm start from WM training",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="bonus added when inside goal radius",
    )
    parser.add_argument(
        "--advance_goal_on_success",
        action="store_true",
        help="shift goal forward by one radius whenever inside the goal",
    )
    parser.add_argument(
        "--reward_scale",
        type=float,
        default=5.0,
        help="scale for distance delta reward",
    )
    ...
    if args.init_policy_zip:
        ...
        print(f"[LOAD] Warm-starting from {args.init_policy_zip}")
        model = algo_cls.load(
            args.init_policy_zip,
            env=vec,
            device=args.device,
            custom_objects=custom_objects,
        )
        model.tensorboard_log = tb_train_dir
    if model is None:
        model = algo_cls(
            policy_id,
            vec,
    ...
    warm_start_steps = int(model.num_timesteps) if args.init_policy_zip else 0
    target_timesteps = int(args.total_timesteps)
    if args.init_policy_zip:
        print(
            f"[TRAIN] Warm start at {warm_start_steps:,}; running "
            f"{target_timesteps:,} new SIM steps (no reset)."
        )

    progress_cb = TqdmProgressCallback(
        total_timesteps=target_timesteps,
        refresh_sec=0.5,
        start_offset=warm_start_steps if args.init_policy_zip else 0,
    )
    callback = CallbackList([progress_cb])

    save_path = os.path.join(run_dir, "models", f"{agent_name}_policy")
    try:
        print(f"[TRAIN] Starting learn for {target_timesteps:,} timesteps")
        learn_kwargs = dict(
            total_timesteps=target_timesteps,
            callback=callback,
            reset_num_timesteps=not bool(args.init_policy_zip),
        )
        if args.a2c:
            learn_kwargs["log_interval"] = args.log_interval
        model.learn(**learn_kwargs)
    except KeyboardInterrupt:
        print("[WARN] KeyboardInterrupt detected; attempting graceful shutdown and checkpoint.")
    finally:
        try:
            print(f"[SAVE] Saving policy to {os.path.abspath(save_path)}")
            model.save(save_path)
        except Exception as save_exc:
            print(f"[ERROR] Failed to save policy: {save_exc}")
        try:
            env_ref = model.get_env()
            ...
            try:
                ...
                print(f"[WARN] VecEnv close raised: {vec_exc}")
